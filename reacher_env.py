...
class ReacherWrapper(DMControlBaseEnv):
    ...
    def reset(self, randomize_target=True, seed=None, options=None):
        self.steps = 0
        if self._use_image:
            obs = Observation()
            if randomize_target:
            ...
            obs.image = np.concatenate(self._image_buffer, axis=0)
        else:
            if randomize_target:
    ...
    def step(self, action):
        ...
        if self._use_image:
            ...
            new_img = self._get_new_img()
            self._image_buffer.append(new_img)
            next_obs.image = np.concatenate(self._image_buffer, axis=0)
        else:
